import { loadGameUI, playerRotation, seats, seatSelected } from './joclogica.js';
import { 
    isDragging,
    MENU_DEPTH,
    CARD_HEIGHT
...
export var config = {
  ...
  // Mides inicials basades en la mida de la finestra
  width: 1000,
  height: 1000,
  scale: {
...
export const TABLE_EDGE_FROM_CENTER = 625 - CARD_HEIGHT/2; // Distància de la vora de la taula des del centre de la taula
export const TABLE_EDGE_CONSTANT = ((2+Math.pow(2,.5))/(1+Math.pow(2,.5))) * TABLE_EDGE_FROM_CENTER;


export var players = {};           // Llista de tots els jugadors actuals del joc
...
//Càmera principal per a aquest reproductor i capturador d'entrada del teclat
export var cam;
var maxZoom;
var floor;
// Crea joc Phaser3
...
function preload() {
  this.load.html('avatar', 'images/boton.html');
  this.load.image('floor', 'images/fons.png');
  this.load.image('tableTop', 'images/mesa.png');
...
function create() {
  ...
  self.input.on('pointermove', function(pointer, currentlyOver) {
    if (pointer.leftButtonDown() && !currentlyOver[0] && isDragging == -1) {
      var camAngle = Phaser.Math.DegToRad(playerRotation); 
      ...
      cam.scrollX -= scrollX;
      cam.scrollY -= scrollY;
      //Actualitza el desplaçament de planta
    ...
    //servidor d'actualització amb la ubicació del punter
  ...
  // Sempre que es canvia la mida de la finestra
...
// Obté paràmetres/consultes d'URL per a un nom i retorna el valor
...
//Obté la llista dels jugadors actuals del servidor
function getPlayerUpdates(self, frames) {
  ...
  self.socket.on('currentPlayers', function (playersInfo) {
    if (seatSelected == true) {   // no creeu la mà fins que es conegui el seient del jugador
...
function removeOldDummyCursors(self, players){
  //eliminar els cursors del llenç si el seu reproductor no existeix
  self.dummyCursors.getChildren().forEach(function(dummyCursor){
  ...
  //elimina els cursors duplicats per a cada jugador
  Object.keys(players).forEach(function(player){
    let playerDummies = [];
    self.dummyCursors.getChildren().forEach(function(dummyCursor){
...
function moveDummyCursors(self){
  //actualitzeu d'alguna manera tots els cursors de cada client
